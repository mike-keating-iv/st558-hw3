...
There are instances where a combination of species and island can
...
```{r}
penguins |> select(species, island, bill_length_mm) |> filter(species == "Chinstrap", island == "Torgersen")
```
...
```{r}
penguins |> select(species, island, bill_length_mm) |> filter(species == "Adelie", island == "Torgersen") |> str()
```
...
penguins |>
  ...
  pivot_wider(names_from = island, values_from = n, 
              values_fill = 0) 
#TODO look up how to cast int columns into double columns
```
...
penguins |> select(species, island, bill_length_mm) |> 
  ...
  mutate(bill_length_mm = case_when(species == "Adelie" ~ 26,
            species == "Gentoo" ~ 30,
            .default = NA))
